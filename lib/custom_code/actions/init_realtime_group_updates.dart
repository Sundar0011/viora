...
Future initRealtimeGroupUpdates() async {
  ...
  final channel = freshRealtimeChannel(client, 'group_updates_realtime_feed');
  ...
  channel.onPostgresChanges(
    ...
    callback: (payload) {
      try {
        ...
        appLog('📡 Realtime Event Type: ${eventType} on table: group');
        ...
        FFAppState().update(() {
          ...
          if (eventType == PostgresChangeEvent.insert ||
              ...
              for (final key in newRow.keys) {
                ...
                switch (key) {
                  ...
                  case 'e_discoverability':
                    existingGroup['e_discoverability'] = newValue.toString();
                    break;
                }
              }

              groups[existingIndex] = existingGroup;
              appLog('✅ Merged data for existing group: $groupId');
            } else {
            ...
            appLog('🗑️ Deleting group with ID: $groupId');
            ...
            if (existingIndex != -1) {
              ...
              appLog('✅ Deleted group: $groupId');
            } else {
              appLog('⚠️ Group $groupId not found for deletion');
            }
          }

          FFAppState().AsGroupList = groups;
          appLog('✅ Updated AsGroupList with ${groups.length} groups');
        });
      } catch (e) {
        appLog('❌ Error in group table callback: $e');
      }
    },
  );
    ...
    callback: (payload) {
      try {
        ...
        if (affectedUserId != currentUserId) {
          appLog('⏭️ Event not for current user, ignoring');
          return;
        }

        final groupId = newRow?['group_id'] ?? oldRow?['group_id'];
        if (groupId == null) {
          appLog('❌ No group_id found in payload');
          return;
        }

        appLog('🔄 Processing user status change for group: $groupId');
        ...
        FFAppState().update(() {
          ...
          if (existingIndex != -1) {
            ...
            if (eventType == PostgresChangeEvent.delete) {
              ...
              final newUserStatus = calculateUserStatus(newRow, existingGroup);
              existingGroup['user_status'] = newUserStatus;
            }

            groups[existingIndex] = existingGroup;
            appLog('✅ Updated user_status for group: $groupId');
          } else {
            appLog('⚠️ Group $groupId not found in app state');
          }

          FFAppState().AsGroupList = groups;
        });
      } catch (e) {
        appLog('❌ Error in group_user_status callback: $e');
      }
    },
  );
        ...
        callback: (payload) {
          try {
            ...
            if (invitedUserId != currentUserId) {
              appLog('⏭️ Invite event not for current user, ignoring');
              return;
            }

            final groupId = newRow?['group_id'] ?? oldRow?['group_id'];
            if (groupId == null) {
              appLog('❌ No group_id found in invite payload');
              return;
            }

            appLog('🔄 Processing invite change for group: $groupId');
              ...
              if (existingIndex != -1) {
                ...
                if (eventType == PostgresChangeEvent.insert) {
                  ...
                  if (isMember) {
                    ...
                    existingGroup['invited_by_profile_picture'] = null;
                    appLog(
                        '✅ Invite accepted, user now joined group: $groupId');
                  }
                } else if (eventType == PostgresChangeEvent.delete) {
                  ...
                  existingGroup['invited_by_name'] = null;
                  existingGroup['invited_by_profile_picture'] = null;
                  appLog('✅ Invite removed for group: $groupId');
                }

                groups[existingIndex] = existingGroup;
                appLog('✅ Updated invite status for group: $groupId');
              } else {
                appLog('⚠️ Group $groupId not found in app state');
              }
            ...
            appLog('❌ Error in group_members_invite callback: $e');
          }
        },
      )
      .subscribe();
