...
class SquaDDSupabaseUser extends BaseAuthUser {
  SquaDDSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> squaDDSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SquaDDSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
